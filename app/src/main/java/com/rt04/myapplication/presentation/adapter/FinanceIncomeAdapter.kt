...
class FinanceIncomeAdapter(private val incomeList: ArrayList<Income>) :
    RecyclerView.Adapter<FinanceIncomeAdapter.MyViewHolder>() {
    class MyViewHolder(val binding: ReportFinanceListBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(income: Income) {
            val jumlah = income.jumlah?.toInt()
